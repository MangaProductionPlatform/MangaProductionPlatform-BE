...
using MangaERP.Publishing.Infrastructure.Persistence;
...
builder.Services.AddSwaggerGen(c =>
    c.SwaggerDoc("v1", new() { Title = "MangaERP - Publishing Service", Version = "v1" }));
...
{
    var context = scope.ServiceProvider.GetRequiredService<PublishingDbContext>();
    int retries = 10;
    while (retries > 0)
    {
        try
        ...
        {
            retries--;
            Console.WriteLine($"Database initialization for PublishingDb failed. Retrying... ({retries} retries left). Error: {ex.Message}");
            if (retries == 0) throw;
